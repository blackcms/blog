...
use BlackCMS\Blog\Services\Abstracts\StoreTagServiceAbstract;
...
class StoreTagService extends StoreTagServiceAbstract
{
    ...
    public function execute(Request $request, Post $post)
    {
        ...
        $tagsInput = collect(json_decode($request->input("tag"), true))
            ->pluck("value")
            ->all();

        if (count($tags) != count($tagsInput) ||
            count(array_diff($tags, $tagsInput)) > 0
        ) {
            $post->tags()->detach();
            foreach ($tagsInput as $tagName) {
                ...
                if ($tag === null && !empty($tagName)) {
                    ...
                    event(
                        new CreatedContentEvent(TAG_MODULE_NAME, $request, $tag)
                    );
                }
