...
use BlackCMS\Blog\Http\Resources\ListPostResource;
use BlackCMS\Blog\Http\Resources\PostResource;
use BlackCMS\Blog\Models\Post;
use BlackCMS\Blog\Repositories\Interfaces\PostInterface;
use BlackCMS\Blog\Supports\FilterPost;
...
class PostController extends Controller
{
    ...
    {
        $data = $this->postRepository->advancedGet([
    ...
    public function getSearch(
        ...
        if ($data["count"] > 0) {
            return $response->setData(
                apply_filters(BASE_FILTER_SET_DATA_SEARCH, $data)
            );
        }
    ...
    public function findBySlug(string $slug, BaseHttpResponse $response)
    {
        ...
        return $response->setData(new PostResource($post))->toApiResponse();
    }
}
