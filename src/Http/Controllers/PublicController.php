...
class PublicController extends Controller
{
    ...
    public function getSearch(Request $request, PostInterface $postRepository)
    {
        ...
        Seo::setTitle($title)->setDescription($title);
    ...
    public function getTag($slug, BlogService $blogService)
    {
        ...
        if (isset($data["slug"]) && $data["slug"] !== $slug->key) {
            return redirect()->to(
                route(
                    "public.single",
                    SlugHelper::getPrefix(Tag::class) . "/" . $data["slug"]
                )
            );
        }
    ...
    public function getPost($slug, BlogService $blogService)
    {
        ...
        if (isset($data["slug"]) && $data["slug"] !== $slug->key) {
            return redirect()->to(
                route(
                    "public.single",
                    SlugHelper::getPrefix(Post::class) . "/" . $data["slug"]
                )
            );
        }
    ...
    public function getCategory($slug, BlogService $blogService)
    {
        ...
        if (isset($data["slug"]) && $data["slug"] !== $slug->key) {
            return redirect()->to(
                route(
                    "public.single",
                    SlugHelper::getPrefix(Category::class) . "/" . $data["slug"]
                )
            );
        }
