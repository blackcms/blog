...
use BlackCMS\Base\Forms\Fields\TagField;
use BlackCMS\Base\Forms\FormAbstract;
use BlackCMS\Blog\Forms\Fields\CategoryMultiField;
...
class PostForm extends FormAbstract
{
    ...
    public function buildForm()
    {
        $selectedCategories = [];
        if ($this->getModel()) {
            $selectedCategories = $this->getModel()
                ->categories()
                ->pluck("category_id")
                ->all();
        }
        ...
        if (!$this->formHelper->hasCustomField("categoryMulti")) {
            $this->formHelper->addCustomField(
                "categoryMulti",
                CategoryMultiField::class
            );
        }
        ...
        $this->setupModel(new Post())
            ...
            ->add("categories[]", "categoryMulti", [
                "label" => trans("addons/blog::posts.form.categories"),
                "label_attr" => ["class" => "control-label required"],
            ...
            ->add("tag", "tags", [
                "label" => trans("addons/blog::posts.form.tags"),
                "label_attr" => ["class" => "control-label"],
                "value" => $tags,
                "attr" => [
                    "placeholder" => trans("addons/blog::base.write_some_tags"),
        ...
        if (count($postFormats) > 1) {
            $this->addAfter("status", "format_type", "customRadio", [
                "label" => trans("addons/blog::posts.form.format_type"),
                "label_attr" => ["class" => "control-label"],
