...
class CategoryRepository extends RepositoriesAbstract implements
    CategoryInterface
{
    /**
     * {@inheritDoc}
    ...
    public function getAllRelatedChildrenIds($id)
    {
        ...
        foreach ($children as $child) {
            $result[] = $child->id;
            $result = array_merge(
                $this->getAllRelatedChildrenIds($child),
                $result
            );
        }
    ...
    public function getFilters($filters)
    {
        ...
        $orderBy = isset($filters["order_by"])
            ? $filters["order_by"]
            : "created_at";
        ...
        return $this->applyBeforeExecuteQuery($this->model)->paginate(
