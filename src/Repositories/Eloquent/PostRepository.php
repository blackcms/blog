...
class PostRepository extends RepositoriesAbstract implements PostInterface
{
     ...
     * {@inheritDoc}
     */
    public function getListPostNonInList(
        array $selected = [],
        $limit = 7,
        array $with = []
    ) {
    ...
    public function getAllPosts(
        ...
        $data = $this->model->with($with)->orderBy("created_at", "desc");
    ...
    public function getFilters(array $filters)
    {
        ...
        if ($filters["categories"] !== null) {
            ...
            $data = $data->whereHas("categories", function ($query) use (
                $categories
            ) {
        ...
        if ($filters["categories_exclude"] !== null) {
            $data = $data->whereHas("categories", function ($query) use (
                ...
                $query->whereNotIn(
                    "id",
                    array_filter((array) $filters["categories_exclude"])
                );
            });
        }
        ...
        if ($filters["exclude"] !== null) {
            $data = $data->whereNotIn(
                "id",
                array_filter((array) $filters["exclude"])
            );
        }
        ...
        if ($filters["include"] !== null) {
            $data = $data->whereNotIn(
                "id",
                array_filter((array) $filters["include"])
            );
        }
        ...
        if ($filters["author"] !== null) {
            $data = $data->whereIn(
                "author_id",
                array_filter((array) $filters["author"])
            );
        }
        ...
        if ($filters["author_exclude"] !== null) {
            $data = $data->whereNotIn(
                "author_id",
                array_filter((array) $filters["author_exclude"])
            );
        }
